...
let doubled = myNumbers.map(function (d) {
	return d * 2
...
let filtered  = myNumbers.filter(function (f) {
	return f > 100
...
let total = myNumbers.reduce(function (acc, t) {
	return acc + t
...
// Last, lets use .forEach to find the index of each item in the array. First, create an empty array called myNumbersIndex. Next, using forEach, push each items index from the myNumbers array to the myNumbersIndex array
 let myNumbersIndex = []
	...
	myNumbers.forEach(function(a,b) {
		return myNumbersIndex .push(b)
...
let forTheLoveOfGeorge = notGeorge.map(function (g) {
	return g = "George"
...
let totallyAwesome = people.reduce(function (acc, t) {
	return acc + t.awesomeLevel
